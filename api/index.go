...
/*
IMPORTANT:
- `icons` map comes from icons_data.go (generated)
- DO NOT redeclare `icons` here
*/
...
func generateSvg(iconNames []string, perLine int, hasTitles bool, align string) string {
	...
	case "center":
		svg = fmt.Sprintf(
			`<svg width="100%%" height="%d" viewBox="0 0 %d %d" xmlns="http://www.w3.org/2000/svg">`,
			scaledHeight, length, height,
		)
	case "right":
		svg = fmt.Sprintf(
			`<svg width="calc(200%% - %dpx)" height="%d" viewBox="0 0 %d %d" xmlns="http://www.w3.org/2000/svg">`,
...
func parseShortNames(names []string, theme string) []string {
	...
	for _, name := range names {
		...
		if full, ok := shortNames[name]; ok {
			if contains(themedIcons, full) {
				result = append(result, full+"-"+theme)
			} else {
...
func iconRoute(r *gin.RouterGroup) {
	r.GET("/icons", func(ctx *gin.Context) {
		...
		if iconParam == "all" {
			...
			names = strings.Split(iconParam, ",")
		}

		iconNames := parseShortNames(names, theme)
